...
def validate_arguments(args):
    ...
    for variant in args.variants:
        match variant:
            case GradientDescentVariant.GD.name:
                count_gd += 1
            case GradientDescentVariant.RGD.name:
                count_rgd += 1
            case GradientDescentVariant.PGD.name:
                count_pgd += 1
            case GradientDescentVariant.CGD.name:
                count_cgd += 1
            case GradientDescentVariant.SGD.name:
                count_sgd += 1

    number_of_variants = len(args.variants)

    number_of_epochs = len(args.epochs)
    number_of_digits = len(args.digits)
    number_of_learning_rates = len(args.learning_rates)
    number_of_loss_functions = len(args.loss_functions)

    number_of_radii = len(args.radii)
    number_of_cutoffs = len(args.cutoffs)
    number_of_coefficients = len(args.coefficients)
    ...
    if not _verify_variant_count_matches_paramerter_count(number_of_variants, number_of_learning_rates):
        raise RuntimeError('The number of variants does not match the number of learning rates supplied.')
    if not _verify_variant_count_matches_paramerter_count(number_of_variants, number_of_loss_functions):
        raise RuntimeError('The number of variants does not match the number of loss functions supplied.')
    if not _verify_variant_count_matches_paramerter_count(number_of_variants, number_of_epochs):
        raise RuntimeError('The number of variants does not match the number of epochs supplied.')
    if not _verify_variant_count_matches_paramerter_count(number_of_variants, number_of_digits):
    ...
    if not _verify_variant_count_matches_paramerter_count(count_pgd, number_of_radii):
        raise RuntimeError('The number of PGD instances does not match the number of radii supplied.')
    if not _verify_variant_count_matches_paramerter_count(count_cgd, number_of_cutoffs):
        raise RuntimeError('The number of CGD instances does not match the number of cutoffs supplied.')
    if not _verify_variant_count_matches_paramerter_count(count_rgd, number_of_coefficients):
...
def build_instances(args):
    # TODO - Use an index-based or zip to iterate over the variants and their parameters.
    configs = RunConfiguration(args.verbose, args.compare)

    for variant in args.variants:
        match variant:
            case GradientDescentVariant.GD:
                configs.append(GradientDescent())
            case GradientDescentVariant.RGD:
                configs.append(RegularizedGradientDescent())
            case GradientDescentVariant.PGD:
                configs.append(ProjectedGradientDescent())
            case GradientDescentVariant.CGD:
                configs.append(ConstrainedGradientDescent())
            case GradientDescentVariant.SGD:
                configs.append(StochasticGradientDescent())
            case _:
